...
func (ctrl *UserRoleController) GetUserPermissions(c *gin.Context) {
	var targetUserID uint

	// Проверяем: нам передали ID в URL или мы смотрим "свои" права?
	userIDParam := c.Param("user")
	if userIDParam != "" {
		id, _ := strconv.ParseUint(userIDParam, 10, 32)
		targetUserID = uint(id)
	} else {
		// Берем ID из токена (middleware уже его туда положила)
		val, _ := c.Get("user_id")
		targetUserID = val.(uint)
	}

	permissions, err := ctrl.userRoleService.GetUserPermissions(targetUserID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Ошибка получения прав"})
		return
	}

	// Перекладываем в чистый DTO (как мы делали раньше)
