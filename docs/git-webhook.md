...
**`POST /api/hooks/git`** (а также литеральный из ТЗ **`POST /hooks/git`** — тот
же обработчик) — открыт без авторизации. `secret_key` принимается как из
JSON-тела (`{"secret_key":"..."}`), так и из `form-data` (`secret_key=...`).
Метод **POST** выбран осознанно: при GET ключ попал бы в URL, access-логи и
историю — это утечка секрета.

> Почему два пути: обратный прокси (nginx/Caddy) проксирует на бэкенд только
> `/api/*`, поэтому в проде рабочий путь — `/api/hooks/git`. Литеральный
> `/hooks/git` из методички доступен при прямом обращении к бэкенду (как на
> демо). Оба сгруппированы под префиксом `hooks`.

При **«грязном» рабочем дереве** (есть незакоммиченные/неотслеживаемые правки)
сервер перед основными командами делает `git status --porcelain`, пишет
**warning** в журнал и в JSON-ответ (поле `warnings`), затем `git clean -fd`
убирает неотслеживаемые файлы (без `-x` — gitignored, напр. `.env`, не
трогаются); отслеживаемые правки отбрасывает обязательный `git reset --hard`.
Локальные изменения на сервере намеренно **не** stash-ятся и не коммитятся —
они могут быть случайными.
...
### Турнкей-демо настоящего `git pull` (рекомендуется для защиты)

```bash
bash scripts/git-webhook-demo.sh
```

Скрипт сам, **не трогая твой репозиторий**, создаёт во временной папке отдельный
git-репозиторий с собственным `origin` (в котором лежит маркер-коммит),
поднимает throwaway Postgres + backend, дёргает `POST /hooks/git` и показывает,
что **маркер-файл реально подтянулся** (`git pull` сделал fast-forward) — плюс
печатает журнал деплоя и негативный кейс (неверный ключ → 403). По завершении
всё убирается автоматически. Это самый наглядный способ показать преподавателю
реальный деплой.
